...
const string strMessageMagic = "Bancorcoin Signed Message:\n";
...
bool CTxOut::IsDust() const
{
    ...
    // so dust is a txout less than 54 uBNCR
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    // Subsidy is cut in half every 1100000 blocks, which will occur approximately every 4 years
    nSubsidy >>= (nHeight / 1100000);
...
void ThreadScriptCheck() {
    RenameThread("bancorcoin-scriptch");
...
void SetGenesisHash() {
        // Genesis Block:
        // CBlock(hash=000000000019d6, ver=1, hashPrevBlock=00000000000000, hashMerkleRoot=4a5e1e, nTime=1390257732, nBits=1d00ffff, nNonce=2083236893, vtx=1)
        ...
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 0x61b04d58 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
...
unsigned char pchMessageStart[4] = { 0x61, 0xb0, 0x4d, 0x58 };
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("BancorcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("BancorcoinMiner : ProcessBlock, block not accepted");
    }
...
{
    printf("BancorcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("bancorcoin-miner");
    ...
    try { while (true) {
        ...
        printf("Running BancorcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("BancorcoinMiner terminated\n");
        throw;
    }
}
