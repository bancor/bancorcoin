...
void QRCodeDialog::setModel(OptionsModel *model)
{
    ...
    // update the display unit, to not use the default ("BNCR")
    updateDisplayUnit();
}
...
{
    QString ret = QString("bancorcoin:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non BNCR unit input in lnReqAmount, we generate the URI with BNCR as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::BNCR, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
